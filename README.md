...
|-------|-------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------|--------|------------------------------------------------------------------|
| `1`   | MPS + `cloud-plugin`                                  | MPS language definition that is used by all examples. The MPS "structure" is used to generate a Java API consumed by all examples. | ✅      | [mps](mps)                                                       |
| `2`   | MPS, `api-gen`                                        | Generated Java API from the MPS language.                                                                                          | ✅      | [University.Schedule.api](mps/solutions/University.Schedule.api) |
| `3a`  | MPS, `json-bulk-access` [EX DASHBOARD FOLDER w/ ktor] | A hand-crafted API that exposes the model contents as a REST API based on MPS as a source for the models                           | 🏗     | [rest-api-json-bulk](rest-api-json-bulk)                         |
| `3b`  | MPS, `model-server`     [QUARKUS/SPRINGBOOT]?         | A hand-crafted API that exposes the model contents as a REST API based on a `model-server` as a source for the models              | 🏗     | [rest-api-model-server](rest-api-model-server)                   |
